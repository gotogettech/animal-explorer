/* =========================================================
   Little Genius Explorer - Full Build
   Tabs + Shape/Color/Number Games + Certificate PNG
   ========================================================= */
...
function showAnimal(an) {
  ...
  if (an.sound) {
    const audio = new Audio(an.sound);
    audio.play().catch(()=>{});
  }
}
...
async function renderLetters(path, gridEl, lang) {
  ...
  letters.forEach(l => {
    ...
    div.addEventListener("click", () => {
      if (l.sound) {
        new Audio(l.sound).play().catch(()=>speak(l.char, lang));
      } else {
        speak(l.char, lang);
      }
    });
...
/* =========================================================
   Games
   - Player selects Shape / Color / Number Game
   - Prompt for name
   - 10 questions
   - Score + progress
...
const gamesMenuBtns = document.querySelectorAll(".game-btn");
...
function startGame(mode) {
  if (!shapesData.length || !colorsData.length) {
    // make sure data loaded first (init loads it, but just in case)
    console.warn("Data not fully loaded yet; waiting...");
  }
  gameMode = mode;
...
function askShapeQuestion() {
  ...
  gameContent.innerHTML = `
    ...
    <p class="mb-4">Tap the <span class="text-indigo-600 font-bold">${correct.name}</span>:</p>
  ...
  options.forEach(opt => {
    ...
    b.innerHTML = `<img src="${opt.icon}" alt="${opt.name}" class="w-20 h-20 mx-auto mb-1 object-contain"><span>${opt.name}</span>`;
...
function askColorQuestion() {
  ...
  gameContent.innerHTML = `
    <h3 class="text-xl font-bold mb-2">${qNum}</h3>
    <p class="mb-4">Tap the color <span style="color:${correct.color}" class="font-bold">${correct.name}</span>:</p>
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4" id="game-opt-wrap"></div>
    <p id="game-feedback" class="mt-4 text-center font-bold"></p>
  `;
  const wrap = document.getElementById("game-opt-wrap");
  options.forEach(opt => {
    const b = document.createElement("button");
    b.className = "p-8 rounded shadow hover:scale-105 transition";
...
function askNumberQuestion() {
  ...
  gameContent.innerHTML = `
    <h3 class="text-xl font-bold mb-2">${qNum}</h3>
    <p class="mb-4">Which number is <span class="text-blue-600 font-bold">${numberToWords(correctNum)}</span>?</p>
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4" id="game-opt-wrap"></div>
    <p id="game-feedback" class="mt-4 text-center font-bold"></p>
  `;
  const wrap = document.getElementById("game-opt-wrap");
...
function finishGame() {
  // show summary & download certificate
  gameContent.innerHTML = `
  ...
  document.getElementById("cert-download-btn").addEventListener("click", () => {
    ...
    // clear player name so next child can enter theirs
    gamePlayerName = "";
  });
...
function generateCertificatePNG({name, score, total, mode}) {
  ...
  // gradient bg with brand colors
  const g = ctx.createLinearGradient(0, 0, w, h);
  g.addColorStop(0, "#fbcfe8");  // pink-200
  g.addColorStop(0.5, "#fef9c3"); // yellow-100
  g.addColorStop(1, "#bae6fd");  // sky-200
  ctx.fillStyle = g;
  ...
  link.download = `LittleGeniusExplorer_Certificate_${name.replace(/\s+/g,'_')}.png`;
  link.href = dataURL;
  link.click();
}
